...
func (as *AdminServer) Run() {
	go func() {
		...
		as.log.Info("PORT 8001 | GET    | /api/events is set up for retrieving events without their request and response bodies")
		as.log.Info("PORT 8001 | GET    | /api/events/:id is set up for retrieving one event with its request and response bodies")
		as.log.Info("PORT 8001 | DELETE | /api/events is set up for deleting the events of a time range")
